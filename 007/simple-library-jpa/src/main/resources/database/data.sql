...
SELECT
    'Детям очень понравились сказки из этой книги' TEXT,
    (SELECT ID FROM BOOKS WHERE TITLE = 'Волк и семеро козлят') BOOK_ID
UNION ALL
SELECT
    'Очень добрые сказки, под них хорошо засыпать :)' TEXT,
    (SELECT ID FROM BOOKS WHERE TITLE = 'Волк и семеро козлят') BOOK_ID
UNION ALL
SELECT
    'Еще какой-то коментарий' TEXT,
    (SELECT ID FROM BOOKS WHERE TITLE = 'Волк и семеро козлят') BOOK_ID
UNION ALL
SELECT
    'Очень крутая книга! Рекомендую!' TEXT,
    (SELECT ID FROM BOOKS WHERE TITLE = 'Чапаев и пустота') BOOK_ID
UNION ALL
SELECT
    'А как по мне, так НЕ очень!' TEXT,
    (SELECT ID FROM BOOKS WHERE TITLE = 'Чапаев и пустота') BOOK_ID
;